	...
	public class CombatTask : IBotTask
	{
		...
		public IEnumerable<IBotTask> Component
		{
			get
			{
				...
				{
					return color.OMilli == 500 && color.RMilli == 0 && color.GMilli == 150 && color.BMilli == 600;
				}
				...
				var listOverviewEntryToAttack =
					memoryMeasurement?.WindowOverview?.FirstOrDefault()?.ListView?.Entry
						...
						?.OrderBy(entry =>
							entry?.Name?.RegexMatchSuccessIgnoreCase(
								@"coreli|centi|alvi|pithi|corpii|gistii")) //Frigate
						?.OrderBy(entry =>
							entry?.Name?.RegexMatchSuccessIgnoreCase(
								@"corelior|centior|alvior|pithior|corpior|gistior")) //Destroyer
						?.OrderBy(entry =>
							entry?.Name?.RegexMatchSuccessIgnoreCase(
								@"corelum|centum|alvum|pithum|corpum|gistum")) //Cruiser
						?.OrderBy(entry =>
							entry?.Name?.RegexMatchSuccessIgnoreCase(
								@"corelatis|centatis|alvatis|pithatis|copatis|gistatis")) //Battlecruiser
						?.OrderBy(entry =>
							entry?.Name?.RegexMatchSuccessIgnoreCase(
								@"core\s|centus|alvus|pith\s|corpus|gist\s")) //Battleship
						?.ThenBy(entry => entry?.DistanceMax ?? int.MaxValue)
						?.ToArray();

				var listOverviewDreadCheck = memoryMeasurement?.WindowOverview?.FirstOrDefault()?.ListView?.Entry
					?.Where(entry => entry?.Name?.RegexMatchSuccess("Dreadnought") ?? true)
					.ToList();

				var armorHitPoitns = memoryMeasurement?.ShipUi?.HitpointsAndEnergy?.Armor;
				var armorRepairers =
					memoryMeasurementAccu?.ShipUiModule?.Where(module =>
						module?.TooltipLast?.Value?.IsArmorRepairer == true);
				...
				else
				{
					if (armorHitPoitns < 830 && armorHitPoitns > 450)
						yield return bot.EnsureIsActive(armorRepairers);
					else if (armorHitPoitns > 900) yield return bot.DeactivateModule(filteredArmorRepairers);
				}
				...
				else
				{
					...
					if (listOverviewEntryToAttack.Count() > 0 && bot?.OwnAnomaly == true)
					{
						...
						var setModuleWeapon =
							memoryMeasurementAccu?.ShipUiModule?.Where(module =>
								module?.TooltipLast?.Value?.IsWeapon ?? false);
						...
						var droneInLocalSpaceIdle =
							droneInLocalSpaceSetStatus?.Any(droneStatus =>
								droneStatus.RegexMatchSuccessIgnoreCase("idle")) ?? false;

						var overviewEntryLockTarget =
							listOverviewEntryToAttack?.FirstOrDefault(entry =>
								!((entry?.MeTargeted ?? false) || (entry?.MeTargeting ?? false)));
						...
						if (NPCtargheted != null && NPCtargheted < TargetCountMax &&
						    ShipManeuverStatus == ShipManeuverTypeEnum.Orbit)
							if (overviewEntryLockTarget.DistanceMax < 60000)
								yield return new LockTarghet {Target = overviewEntryLockTarget};
					}
